...
public class FlockRollback implements Runnable {
    ...
    private static final String DELETE_FROM_HISTORY_SQL =
            "DELETE FROM \"__flock_migrations_history\" WHERE migration_id = '%s';";
    // The migration_id is timestamp-prefixed (yyyyMMddHHmmss_name), so lexical DESC
    // gives the latest applied.
    private static final String FETCH_LATEST_MIGRATION_ID_SQL =
            "SELECT migration_id FROM \"__flock_migrations_history\" ORDER BY migration_id DESC LIMIT 1;";
    ...
    private void performRollback(String migrationId) {
        ...
        try {
            ...
            try (var statement = conn.createStatement()) {
                ...
                var deleteHistorySql = String.format(DELETE_FROM_HISTORY_SQL, migrationId);
    ...
    private Optional<String> getLatestAppliedMigration() throws SQLException {
        ...
        var lastAppliedMigration = App.connection.createStatement().executeQuery(FETCH_LATEST_MIGRATION_ID_SQL);
